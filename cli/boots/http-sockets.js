...
(async () => {
  ...
  const http = (0, import_http.initHttpServer)({ returnInstance: true, modelManager, httpControllers, phoenixHttpConfig, onMessage: (message) => console.log(message) });
  (0, import_web_sockets.initSocketsServer)({ http, modelManager, libraryManager, socketsControllers, phoenixSocketsConfig, onError: (error) => console.error(error) });
})();